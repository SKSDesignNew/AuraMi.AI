...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  CredentialsSignin: 'Sign in failed. Please check your email and try again.',
...
function LoginContent() {
  ...
  // Pick up NextAuth error from query params (e.g. /login?error=Configuration)
  useEffect(() => {
  ...
  // Fetch actually configured providers from NextAuth
  useEffect(() => {
    ...
    <main className="min-h-screen flex items-center justify-center bg-bg px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-card rounded-card p-6 border border-[rgba(0,245,255,0.08)] space-y-4">
          ...
          {/* Divider — only show if OAuth buttons are present */}
          {hasOAuth && (
...
export default function LoginPage() {
  return (
    <Suspense fallback={
      <main className="min-h-screen flex items-center justify-center bg-bg px-4">
        <div className="text-text-500 font-body text-sm">Loading...</div>
      </main>
    }>
      <LoginContent />
    </Suspense>
